...
| T-009 | WI-mui | 인증서·탭·다운로드·분리모드·아이콘 | 구현 완료·창 미표시 원인 수정 (검증 대기) |
| T-010 | WI-361 | 손쉬운 사용 권한 온보딩 랜딩 창 + AppIcon 불투명 배경 | 구현 완료 (수동 검증 대기: OFF→ON 재토글 후 허용됨 전환 확인) |
| T-011 | — | 다운로드 진행 트레이 v2 (임시파일 rename/삭제·용량/속도/남은/경과) + ESC 닫기 확정 + 툴바 ✕·메뉴바 플로팅 토글 + 사이트 제목 표시 | 구현 완료 (수동 검증 대기: 트레이 완성형·ESC·닫기·제목·새창) |
| T-012 | — | Final 아이콘 교체 (Dock Light 계열 10종 + 메뉴바 Black 템플릿 18/36) | 구현 완료 (수동 검증 대기: Dock·메뉴바 라이트/다크) |
