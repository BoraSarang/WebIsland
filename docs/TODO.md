...
1. ~~실행 후 창 미표시 원인 확정~~ → **완료**.
2. ~~TC-MAN + LAN 3종 수동 검증~~ → **완료** (사용자 검증 통과, T-009~T-012).
3. ~~원격 설정 시 푸시·PR~~ → **완료** (현 분기 푸시済み, 구 9개 분기는 병합済み·정리 대기).
4. 영문 로케일 수동 확인 (T-016): 시스템 언어 영문 시 Dock/Finder `Web Island` 표시.
